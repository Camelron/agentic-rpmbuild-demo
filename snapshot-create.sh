...
Create /var/lib/kata/snapshots/POD_NAME from the Pod's Ready Kata sandbox.
...
snapshot_path="/var/lib/kata/snapshots/$pod_name"
...
kubectl node-shell "$node_name" -- \
    ...
    sh -ec '
        ...
        if [ "$DRY_RUN" = true ]; then
            printf "Dry run: kata-ctl snapshot create --sandbox-id %s --path %s\n" \
                "$sandbox_id" "$SNAPSHOT_PATH"
            exit 0
        fi

        mkdir -p /var/lib/kata/snapshots
